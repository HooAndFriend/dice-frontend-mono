...
const GanttContainer = ({ data }: PropsType) => {
  ...
  const calculatePositionAndWidth = (
    ...
    const startDate = months.length
      ? dayjs(months[0].replace('.', '-'), 'YYYY.M').startOf('month')
      : dayjs().startOf('year')

    const endDate = months.length
      ? dayjs(months[months.length - 1].replace('.', '-'), 'YYYY.M').endOf(
          'month',
        )
      : dayjs().endOf('year')
      ...
      <div className="flex">
        <div className="overflow-x-auto">
          <table className="bg-white border-collapse table-fixed min-w-[350px]">
            <thead>
              <tr>
                <th className="p-2 text-left bg-white border min-w-[350px]">
                  스프린트
                </th>
              </tr>
            </thead>
              ...
              {processedData.updatedData.map((item) => {
                ...
                return (
                  <Fragment key={item.epicId}>
                      ...
                      >
                        <div className="relative w-full h-6 bg-gray-200">
                          <div
                            ...
                            onMouseLeave={() =>
                              setHoveredTask({ range: '', index: '' })
                            }
                          >
                            {hoveredTask.range &&
                              hoveredTask.index === `${item.epicId}` && (
                                <div className="absolute top-[-1.5rem] left-0 bg-black text-white text-xs p-1 rounded w-[160px] text-center">
                                  {hoveredTask.range}
                                </div>
                              )}
                          </div>
                    ...
                    {expandedRows.includes(item.epicId) &&
                      item.ticket.map((subItem) => {
                        const subPosition = calculatePositionAndWidth(
                          subItem.createdDate,
                          subItem.dueDate,
                        )
                        return (
                            ...
                            >
                              <div className="relative w-full h-6 bg-gray-200">
                                <div
                                  ...
                                  onMouseEnter={() =>
                                    setHoveredTask({
                                      range: `${subItem.createdDate.replace(
                                        '-',
                                        '.',
                                      )} - ${subItem.dueDate.replace(
                                        '-',
                                        '.',
                                      )}`,
